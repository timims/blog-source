...
我们的分析任务经常是“一个父任务 + 多个传感器子查询”。例如一个场景下 20 个传感器，就会拆成 20 个并行子任务。  当多个父任务同时提交时，问题开始显现：1. 子任务数量快速膨胀，线程池和数据库连接池竞争激烈。  2. 父任务需要等待所有子任务 `join()` 后才能进入下一步，尾延迟受最慢子任务影响。  3. Java 端聚合结果容器变大，内存压力上升。  这推动我们思考：
